...
- `scripts/create-vm.sh` — provisions the VM (disk, VirtIO ISO, define domain, attach Windows install media) and `--finalize` (eject media + clean snapshot). Windows 10 (BIOS) by default; `--win11` fills the template's firmware slots with UEFI + Secure Boot + emulated TPM 2.0 (needs `swtpm` + OVMF) and bumps defaults to 6 GB / 64 G. `--auto-finalize` waits for the post-provisioning power-off (a *stable* "shut off", distinguishing it from Setup's reboots) and finalizes automatically.
