...
- `bin/create-vm` — provisions the VM (disk, VirtIO ISO, define domain, attach Windows install media) and `--finalize` (eject media + clean snapshot). Windows 10 (BIOS) by default; `--win11` fills the template's firmware slots with UEFI + Secure Boot + emulated TPM 2.0 (needs `swtpm` + OVMF), bumps defaults to 6 GB / 64 G, and **adds a second qcow2 disk** (`${GAME_DISK}` slot) for the game. `--auto-finalize` waits for the post-provisioning power-off (a *stable* "shut off") and finalizes automatically.

**Windows 11 NTFS game disk.** The DU updater fails on the VirtIO-FS share on Win11 ("volume does not contain a recognized file system" — it needs a real NTFS volume; works on Win10). So on Win11 only: `create-vm` adds a second disk, `Setup-Guest.ps1` (`Initialize-GameDisk`) formats it NTFS labelled `DUGAME`, and `Start-Updater.ps1` installs/updates the game on that disk then `robocopy /MIR` mirrors it to the share so the files reach Linux (same place Win10 puts them). Win10 has no second disk → unchanged. Don't enable `CLEAN_SNAPSHOT` on Win11 (a revert would wipe the game disk).
